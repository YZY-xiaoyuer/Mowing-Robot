...
static int accel_self_test(long *bias_regular, long *bias_st)
{
    ...
    for(jj = 0; jj < 3; jj++) {
        ...
        if (st_shift[jj]) {
            ...
            printf("st_shift_cust:%f (%f,%f) \r\n", st_shift_cust, test.min_g, test.max_g);
            result |= 1 << jj;
        }
    }
 ...
 */
int mpu_run_self_test(long *gyro, long *accel) //--用于获得当前各轴的角速度和加速度，并且反馈芯片的状态。
{
#ifdef MPU6050
...
//--100hz读取速率。100hz采样率的前提下，读取了一百多万个数据，
//--计算得到每10ms yaw角平均偏移了5.34826E-05,
//--如果需要，根据实际的采样读取时间，进行数据补偿
uint8_t mpu_dmp_get_data(float *pitch, float *roll, float *yaw)
{
    ...
    unsigned char more;
    long quat[4];
    int num = dmp_read_fifo(gyro, accel, quat, &sensor_timestamp, &sensors, &more);
    if(num)
		 return num;
