#!/usr/bin/env python
import rospy
from msg_srv_act.srv import S_along_edge_server,S_map_server,S_location_pose_server
...
def Rqueset_Along_Edge_Plan():
    ...
    if server_is_on == False:
        ...
        return
    try:
        request_along_edge_plan = rospy.ServiceProxy('along_edge_server', S_along_edge_server)
        now_pose = Request_Now_Pose()
        now_pose_x = now_pose.pose[0] + 0.1
        now_pose_y = now_pose.pose[1] - 0.5
        start_point = [now_pose_x, now_pose_y]

        resp1 = request_along_edge_plan(start_point)
        print(resp1)
        return resp1
    except rospy.ServiceException as exc:
        print("Service call failed!  because:" + str(exc))
